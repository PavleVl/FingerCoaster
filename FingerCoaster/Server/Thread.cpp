...
Thread::Thread(qintptr newSocketFd, QObject* parent)
    : QThread(parent), threadSocket(new QTcpSocket(this)) {
  ...
  connect(threadSocket, SIGNAL(readyRead()), this, SLOT(readyRead()),
          Qt::DirectConnection);
  connect(threadSocket, SIGNAL(disconnected()), this, SLOT(disconnected()),
          Qt::DirectConnection);
  connect(parent, SIGNAL(sendMessage(QByteArray, qintptr)), this,
          SLOT(onSendMessage(QByteArray, qintptr)), Qt::DirectConnection);
  connect(parent, SIGNAL(endConnection()), this, SLOT(forceCloseConnection()),
          Qt::DirectConnection);
...
void Thread::onSendMessage(QByteArray message, qintptr targetSocketFd) {
  if (threadSocket->isWritable() &&
      (targetSocketFd == socketFd || targetSocketFd == 0)) {
...
void Thread::readyRead() {
  ...
  if (text.contains(QString::fromStdString("clientProgress:"))) {
    ...
    unsigned curGameProgress =
        text.split(QString::fromStdString("-")).at(1).toUInt();
