...
Server::Server(QObject* parent) : QTcpServer(parent), inGame(false) {
  ...
  myUsername =
      QString::fromStdString(sc.giveUsername()) + QString::fromStdString(sufix);
...
void Server::startServer() {
  if (!this->listen(QHostAddress::Any, SERVER_PORT)) {
    ...
    std::cout << "Server has been started "
              << this->serverAddress().toString().toStdString() << std::endl;
  }
}
...
void Server::updateClientsGameProgress(qintptr socketFd,
                                       unsigned curClientProgress) {
  std::cout << "Usao sam u update game progress" << socketFd << " "
            << curClientProgress << std::endl;
  QPair<qintptr, QString> clientKey =
      qMakePair(socketFd, usernames.find(socketFd).value());
...
void Server::broadcastProgress() {
  ...
  while (it != progress.end()) {
    msgBuff += it.key().second + QString::fromStdString("-") +
               QString::number(it.value()) + QString::fromStdString(":");
