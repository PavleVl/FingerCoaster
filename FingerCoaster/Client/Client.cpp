...
Client::Client(QString name, QObject* parent)
  ...
  // treba postaviti putanju do fajla i randomTextFlag na false i pozvati f-ju
  // loadText()
...
void Client::readyRead() {
  ...
  if (buff.contains(QString::fromStdString("text"))) {
    std::cout << "I've caught text initialisation " << buff.toStdString()
              << std::endl;
    std::cout << "Buffer: " << buff.toStdString() << std::endl;
    std::string filename =
        buff.split(QString::fromStdString(":")).last().toStdString();
...
void Client::printError(QAbstractSocket::SocketError socketError) {
  switch (socketError) {
    ...
    case QAbstractSocket::HostNotFoundError:
      std::cerr << "The host was not found,please check the hostname and port"
                << std::endl;
      break;
    case QAbstractSocket::ConnectionRefusedError:
      std::cerr << "The connection was refused. "
                   "Make sure the server is running "
                   "and check that the hostname and port settings are correct"
                << std::endl;
      break;
    default:
      std::cerr << "The following error occured: "
                << tcpSocket->errorString().toStdString() << std::endl;
  }
  emit dontShowLobby();
}
...
void Client::updateProgress(unsigned curProgress) {
  ...
  if (tcpSocket->isWritable()) {
    QString msg = QString::fromStdString("clientProgress:") + username +
                  ...
                  QString::number(curGameProgress);
