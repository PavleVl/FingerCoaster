...
Scoreboard::Scoreboard(QWidget* parent)
    : QDialog(parent), ui(new Ui::Scoreboard) {
  ui->setupUi(this);
  ...
  ui->vr_user->setText(
      QString::fromStdString(Scoreboard->savedScoreboard->giveUsername()));
...
void Model::populateData(const QVector<QString>& place,
                         const QVector<QString>& date,
                         const QVector<QString>& wpm,
                         const QVector<QString>& acc) {
  _place.clear();
  _place = place.toList();
  _date.clear();
  _date = date.toList();
...
auto Model::headerData(int section, Qt::Orientation orientation, int role) const
    -> QVariant {
