...
createroom::createroom(QWidget* parent)
  ...
  ui->num_players_label->setStyleSheet(
      QString::fromStdString("font-weight: bold"));
...
auto createroom::noRBChecked() -> bool {
  if (!ui->rb_easy->isChecked() && !ui->rb_medium->isChecked() &&
      !ui->rb_hard->isChecked())
    return true;
...
void createroom::on_create_lobby_button_clicked() {
  if (noRBChecked()) {
    ui->err_label->setText(
        QString::fromStdString("You need to select difficulty"));
    ui->err_label->setStyleSheet(
        QString::fromStdString("QLabel {background-color: "
  ...
  this->close();
  emit startServer(unsigned(ui->num_players_cb->currentIndex()) + 1,
                   choosenDifficulty());
  emit openLobby();
}
