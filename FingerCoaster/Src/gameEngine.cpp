...
GameEngine::GameEngine() {
  ...
  connect(menuScene->getScoreboardButton(), SIGNAL(clicked()), this,
          SLOT(showScore()));
  connect(menuScene->getJoinButton(), SIGNAL(clicked()), this,
          SLOT(joinRoom()));
  connect(menuScene->getCreateRoomButton(), SIGNAL(clicked()), this,
          SLOT(createRoom()));
...
void GameEngine::joinRoom() {
  JoinPopUp joinPopup;
  connect(&joinPopup, SIGNAL(startClient(QString)), this,
          SLOT(startClient(QString)), Qt::DirectConnection);
  connect(&joinPopup, SIGNAL(openLobby()), this, SLOT(startLobby()),
          Qt::DirectConnection);
...
void GameEngine::startClient(QString key) {
  ...
  connect(ourClient, SIGNAL(startGame()), this, SLOT(setGameScene()),
          Qt::DirectConnection);
}
...
void GameEngine::startServer(unsigned maxPlayers, unsigned difficulty) {
  ...
  connect(ourServer, SIGNAL(serverShutdown()), this, SLOT(reInitServer()),
          Qt::DirectConnection);
...
void GameEngine::startLobby() {
  ...
  if (ourServer != nullptr) {
    connect(ourServer, SIGNAL(updateLobbyList(QString)), &ourLobby,
            SLOT(addPlayer(QString)), Qt::DirectConnection);
    connect(&ourLobby, SIGNAL(closeServerConnections()), ourServer,
            SLOT(blockConnections()), Qt::DirectConnection);
    connect(&ourLobby, SIGNAL(popUpForcedClose()), ourServer,
            SLOT(forceCloseTheServer()), Qt::DirectConnection);
    ...
    connect(&ourLobby, SIGNAL(setGameScene()), this, SLOT(setGameScene()),
            Qt::DirectConnection);
    connect(&ourLobby, SIGNAL(startGameForClients()), ourServer,
            SLOT(startGameForClients()), Qt::DirectConnection);
  } else {
    // Podeseno klijentsko okruzenje
    ourLobby.setIsClient();
    connect(&ourLobby, SIGNAL(joinPopupForcedClose()), this,
            SLOT(forceCloseTheClientConnection()), Qt::DirectConnection);
    connect(this, SIGNAL(forceCloseClient()), ourClient,
            SLOT(forceCloseClient()), Qt::DirectConnection);
    connect(ourClient, SIGNAL(dontShowLobby()), &ourLobby,
            SLOT(dontShowLobby()), Qt::DirectConnection);
    connect(ourClient, SIGNAL(rewriteUsernames(QVector<QString>*)), &ourLobby,
            SLOT(rewriteUsernames(QVector<QString>*)), Qt::DirectConnection);
    // connect(ourClient,SIGNAL(closeClientLobby()),&ourLobby,SLOT(dontShowLobby()),Qt::DirectConnection);
    connect(ourClient, &Client::closeClientLobby, &ourLobby,
            &Lobby::closeLobbyStartGame, Qt::DirectConnection);
    connect(&ourLobby, SIGNAL(startClientGame()), this, SLOT(setGameScene()),
            Qt::DirectConnection);
  }
...
void GameEngine::setGameScene() {
  ...
  connect(gameDialog, SIGNAL(gameDialogClosing()), this, SLOT(reOpenMainMenu()),
          Qt::DirectConnection);

  if (ourServer != nullptr) {
    connect(gameDialog, SIGNAL(shutdownServer()), ourServer,
            SLOT(forceCloseTheServer()), Qt::DirectConnection);
    connect(ourServer, SIGNAL(populateGame(QVector<QString>*)), gameDialog,
            SLOT(populateGame(QVector<QString>*)), Qt::DirectConnection);
    connect(gameDialog, SIGNAL(updateProgress(uint)), ourServer,
            SLOT(setMyProgress(uint)), Qt::DirectConnection);
    connect(ourServer, SIGNAL(changeCurGameProgress(QVector<uint>*)),
            gameDialog, SLOT(updateCurGameProgress(QVector<uint>*)),
            Qt::DirectConnection);
